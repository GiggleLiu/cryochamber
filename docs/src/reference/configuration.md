...
wait_timeout = 14400             # Default `cryo-agent receive --wait` timeout in seconds (cap 86400)
...
| `wait_timeout` | `14400` | Optional. Default timeout in seconds for `cryo-agent receive --wait` when the agent doesn't pass `--timeout`. The daemon caps any value at `86400` (24 h). |
