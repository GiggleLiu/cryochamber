...
- **Daemon-authored stand-in replies.** When a wake ends without any agent-authored outbox message, the daemon writes a `from: cryochamber` message so operators always see at least one update. Retryable runner failures before the agent sends anything or claims an inbox batch defer that stand-in reply until the bounded retry loop is exhausted.
...
- **Crash handling via retry.** Retryable agent-runner exits before any outbox message or inbox claim get a short in-daemon retry loop with increasing gaps. If the final attempt still fails, or if the agent fails after claiming TODO work, the daemon records the crash, marks each claimed TODO done, and adds a fresh attempt-suffixed retry TODO with an exponential delay (`2^k` minutes, capped at 1 day). The original claim is terminal — TODO retries are always *new* items with fresh IDs. `EventLogger` is still finalized on every outcome.
