...
**Agent Console** 是 `cryohub` 提供的网页界面——一个手机优先、可安装的应用，用来阅读和操控 hub 所知的每一个 chamber。每个 chamber 都有一条容纳报告和指令的主消息流，并用专注的全屏话题视图处理后续追问；chamber 的控制项一步之遥。
...
消息正文支持 Markdown，包括表格和围栏代码块，也支持单美元符号包围的行内 LaTeX 与双美元符号包围的块级 LaTeX。控制台在浏览器中渲染并净化这些内容。

需要针对某份报告集中追问时，可以在话题中回复。每次收到新的话题跟进，智能体都会同时获得话题根消息和回复历史；它的答复会自动回到同一话题。在当前对话得到回复前，其他话题和主消息流中的消息会继续等待。

点击 **Reply in thread**、回复数量或新话题活动链接，会打开全屏话题视图。原消息位于顶部，回复依次显示在下方，同一个消息输入框固定在底部。点击 **Back to stream** 或按 Escape 返回主消息流。打开话题期间，主消息流中的草稿和上传仍可继续使用；每个话题也会分别保留自己的文字草稿和已完成上传。

如果某条话题回复也应出现在 chamber 的主消息流中，请使用 **Share to stream**。此操作会在发件箱中创建一份用于展示的副本，不会向智能体发送新指令，也不会将它唤醒。

## 添加附件

可以点击回形针、把文件拖入编辑框，或从剪贴板粘贴文件。每条消息最多暂存 10 个文件，每个文件不得超过 25 MB。发送前可以看到图片预览、上传进度、移除和重试操作。消息可以只有附件，无需填写文字。

已发送的文件显示为下载卡片。文本文件可以直接预览；PDF 使用浏览器内置阅读器，不支持时可下载后打开。

已上传完成的文件引用会随草稿保存，并在重新加载后恢复。排队中或上传中的文件字节只存在于当前浏览器标签页中；如果上传完成前重新加载页面，需要重新添加这些文件。
...
在 public 模式下，每个凭证（访客与 owner 一视同仁）的发送与上传都受限流：突发额度 5 次，之后每分钟 10 次；超出后 hub 返回 `429`，并带上 `Retry-After` 头。收件箱消息可能唤醒智能体，上传则会占用 owner 的磁盘，因此限流可以防止邀请链接推高账单或用文件塞满 chamber。分享到主消息流不会唤醒智能体。
...
访问 token、hub 记录的你的名字、每个 chamber 的已读水位、文本草稿、已上传完成的附件引用，以及一小份最近消息缓存，都存放在该 hub 源（origin）的 `localStorage` 中。排队中和上传中的文件字节不会保存。退出登录会清除这些数据。消息正文在客户端渲染，并在进入 DOM 之前经过净化。
