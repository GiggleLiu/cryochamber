...
仪表盘也支持内联话题。一次 `cryo-agent receive` 或 `cryo-agent dialog` 只领取一段对话：第一条待处理话题中的消息，或主消息流中未加入话题的待处理消息。领取后，智能体必须先回复当前对话，才能领取另一段对话。对于话题跟进，智能体会同时获得根消息和之前的回复作为上下文；它下一次发送的消息会自动回到同一话题。
