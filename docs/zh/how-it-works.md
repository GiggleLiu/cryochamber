...
仪表盘会在专注视图中打开每个话题。一次 `cryo-agent receive` 或 `cryo-agent dialog` 只领取一段对话：第一条待处理话题中的消息，或主消息流中未加入话题的待处理消息。领取后，智能体必须先回复当前对话，才能领取另一段对话。对于话题跟进，智能体会同时获得根消息和之前的回复作为上下文；它下一次发送的消息会自动回到同一话题。

把话题回复分享到主消息流只会在发件箱中创建一份展示副本。它不会向本地收件箱添加消息，因此不会唤醒智能体，也不会为智能体创建新工作。
...
Daemon 在归档已领取的对话前，会持久化回复义务。进程被强制结束后，下次启动
