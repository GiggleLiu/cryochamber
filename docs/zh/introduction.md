...
[Cryohub](./reference/cli.md#hub-cryohub) 提供 **[Agent Console](./agent-console.md)**：每个 chamber 都有主消息流和专注的话题视图，并提供状态、TODO、笔记、日志尾部与生命周期控制，手机和桌面浏览器都能用。它内嵌在 `cryohub` 二进制中——无需安装。可以通过邀请链接把单个 chamber 分享给他人，或用 [`cryo-zulip`](./reference/cli.md#zulip-sync-cryo-zulip) 把它桥接到 Zulip。
