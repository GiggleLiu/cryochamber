...
test('register maps chambers to subscriptions with stable numeric ids', async () => {
  ...
  const again = await new HubClient({ ...OPTS, fetch: fetchFn }).register()
...
describe('chamber liveness', () => {
  ...
  test('register carries the hub liveness onto each subscription', async () => {
    const c = new HubClient({ ...OPTS, fetch: mockFetch(() => chambers) })
  ...
  test('a hub that reports no liveness leaves it undefined rather than asleep', async () => {
    const c = new HubClient({ ...OPTS, fetch: mockFetch(() => [{ id: 'cham-a', name: 'alpha' }]) })
...
test('a second client instance agrees on the number for the same mailbox id', async () => {
  ...
  const a = new HubClient({ ...OPTS, fetch: fetchFn })
  await a.register()
  ...
  const b = new HubClient({ ...OPTS, fetch: fetchFn })
  await b.register()
...
test('sendMessage posts body with CSRF header and 401 throws an auth error', async () => {
  ...
  const denied = new HubClient({ ...OPTS, fetch: mockFetch(() => new Response('', { status: 401 })) })
...
test('an unresolvable project name is marked as a client-side 404, not a server one', async () => {
  ...
  const c = new HubClient({ ...OPTS, fetch: mockFetch(() => new Response('', { status: 500 })) })
...
describe('owner chamber routes', () => {
  ...
  test('chamberStatuses re-reads the same three flags for a status event', async () => {
    const c = new HubClient({ ...OPTS, fetch: mockFetch(() => [{ id: 'cham-a', name: 'alpha', completed: false, archived: true, has_open_question: false }]) })
    await c.register()
...
describe('the 401 hook and authenticated blobs', () => {
  ...
  test('sendMessage posts only the body — the hub stamps the sender', async () => {
    const fetchFn = mockFetch((url) => (url.endsWith('/send') ? { ok: true, id: 'inbox/x.md' } : [{ id: 'cham-a', name: 'alpha' }]))
    const c = new HubClient({ token: 't', fetch: fetchFn })
    await c.register()
    await c.sendMessage('alpha', 'hi')
    const sendCall = vi.mocked(fetchFn).mock.calls.find(([u]) => String(u).endsWith('/send'))!
