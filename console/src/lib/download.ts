/** Shared authenticated-attachment helpers (message clicks + deep links). */
import { ZulipApiError } from '../api/client'

/** Hub attachment routes: /api/chambers/{id}/files/{name}. Like Zulip uploads
 * they need the Authorization header, so a plain navigation would 401. */
export const HUB_FILES_RE = /^\/api\/chambers\/[^/]+\/files\//

/** Throws ZulipApiError, not a bare Error: an attachment that comes back 401 is
...
/** Last path segment of an upload href (query/fragment stripped), URL-decoded. */
...
/** Fetch an upload with auth and save it. Throws on fetch failure. */
