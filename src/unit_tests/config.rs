...
/// Unknown keys in `cryo.toml` (removed options, typos, forward-compat) are
/// ignored on load and dropped on save, not a load failure.
#[test]
fn unknown_config_key_is_ignored() {
    let config: CryoConfig = toml::from_str("agent = \"mock\"\nno_such_key = 600\n").unwrap();
    assert_eq!(config.agent, "mock");
    let out = toml::to_string(&config).unwrap();
    assert!(!out.contains("no_such_key"));
}
