...
- **Bounded TODO list in the wake prompt**: the per-session prompt renders TODOs via `TodoFile::display_for_prompt`, which always shows pending/claimed items but folds all except the 3 most recent completed items behind a count (done items are never deleted, so the full list grows with chamber age and would be re-injected on every wake). `cryo-agent todo list` still returns the full list.
