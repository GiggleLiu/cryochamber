...
- **The Agent Console is the hub's only web surface**: the bundled `web_shell.html` dashboard, `templates/web.css` and `templates/vendor/*` (KaTeX, marked, DOMPurify, the woff2 fonts) are gone, along with `hub::routes::pages` and `hub::routes::fonts`. The hub registers no page routes at all; `hub::routes::console::serve` is the router's unconditional fallback and owns everything outside `/api`. `console_dir` is now an *override* — unset resolves to `hub::paths::global_console_dir()` (`~/.cryo/console`), which is what `make console-install` writes, so a normal install configures nothing. A console root with no `index.html` answers pages with a 503 setup page naming the directory and the command; a missing hashed asset stays a 404, because answering it with HTML would break the module loader. Do not reintroduce a server-rendered dashboard, and do not vendor client-side libraries into `templates/` — the console bundles its own through npm.
