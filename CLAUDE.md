...
- **Protocol prompt guardrails**: The closing-ritual redundancies in `templates/protocol.md` are intentional. Keep prompt-visible reminders that `send`, `todo add --at`, and `hibernate` are separate actions; a `receive`/`dialog`-claimed inbox batch must be followed by a `send`; TODOs, not notes/plans/hibernate, schedule wakes; and `NOTES.md` is durable memory only. The shortcut warnings and Core Invariants block guard known agent failure modes, so do not optimize them away unless a replacement is tested against those same failures.
