...
| `message` | File-based inbox/outbox message system. Agent-side `cryo-agent receive` goes through daemon IPC and claims one conversation, grouped by the first pending message's `thread_id` or the unthreaded main stream. It archives that group into `messages/inbox/archive/` immediately via `MessageStore`. The current session tracks the conversation; a durable obligation backs restart notification. Operator `cryo receive` is separate: it reads messages from `messages/outbox/`. |
...
2. **Every inbox message is answered — by the agent or by the
   ...
   conversation still unanswered, the daemon writes the fallback reply from
   `daemon_unanswered_reply_text` for that conversation
   ...
   startup finishes any partial archive. If no durable reply receipt exists in
   outbox or its archive, startup writes an interruption notice in the claimed
   thread when applicable. The sender decides
...
4. **Claim/consumption is terminal — a picked-up message or TODO
   ...
   succeeded.** An
   inbox conversation read by `cryo-agent receive` or `cryo-agent dialog` is
   archived into `messages/inbox/archive/` immediately and is never re-delivered or
...
- **Attachments are a channel concern, not an agent concern**: the agent only ever deals in chamber-local file paths — it reads pulled attachments from `messages/attachments/` and links to local files when sending. Uploading, downloading, and per-server markdown quirks live in the sync binary. The Console composer stages at most 10 attachments of at most 25 MB each, supports picker/drop/paste, previews, removal, progress, retry, and file-only sends, and uploads them serially through the existing authenticated route. It persists ready upload references with the text draft, but never browser `File` bytes for queued or in-flight uploads. Notably `externalize_local_links` rewrites upload links to **absolute** URLs and strips the `!` from `![alt](...)`; both are required for an inline preview (verified on Zulip 11.4): the preview pass only matches absolute upload URLs, and CommonMark image syntax renders literally before server 12.0 (feature level 437). Do not push channel-specific syntax into `cryo-agent send` or the protocol; do not let either sync implementation upload anything outside the chamber, under `.cryo/`, or named `cryo.toml` (these hold bot or provider API keys).
- **Inbox and thread contract**: There is no `cryo-agent reply`. Wake prompts do not include inbox contents; the daemon only checks whether inbox files exist so it can surface a notice in the session prompt. During an agent session, `cryo-agent receive` and `cryo-agent dialog` ask the daemon to claim one conversation: all currently pending messages whose `thread_id` matches the first pending message, or all unthreaded messages when the first message has none. The daemon archives that group immediately. A durable pending obligation fixes the active conversation, so later claims can extend it with new messages from the same thread but cannot claim a different thread or the main stream before a reply. A new thread claim includes the root and full thread history even when a `dialog --last` or `--since` filter would omit part of it; subsequent dialog reads filter to the active thread and apply the requested limit normally. Bodies returned by `receive` and `dialog`, including thread context, rewrite `/api/chambers/.../files/...` links to chamber-local `messages/attachments/...` paths. The next successful `cryo-agent send` is the reply by definition and automatically inherits the active `thread_id`; daemon fallback and interruption replies do the same. Archive-on-claim is terminal and inbox messages are never replayed. Only TODOs have retry semantics. If the human still wants action, they resend.
...
- **API to manipulate local message files**: `src/channel/store.rs::MessageStore`.
  - `MessageStore` is the local mailbox API for directory setup, archive-stable message lookup, inbox/outbox reads, thread context, and archiving. Explicit thread reads currently scan all mailbox files. Keep that simple until measured history size requires an index; do not claim a thread-read latency SLO.
  - Agent-side `cryo-agent receive` sends `socket::Request::Receive` through `daemon_client::send_checked_request`; during an active session, the daemon selects and archives one inbox conversation through `MessageStore` and records the reply obligation in session memory and a durable journal. `cryo-agent dialog` uses the same claim path.
