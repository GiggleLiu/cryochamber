...
| `cryo-agent` | Agent-side IPC/utility CLI — `hibernate`, `send`, `receive`, `alert`, `time`, `todo` (used by the spawned agent, not by operators; most commands send requests to the daemon via socket, while `time` is local) |
