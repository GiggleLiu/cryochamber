...
| `chamber_registry` | Durable user-level registry of known chamber directories (`~/.cryo/chambers.json` by default; `CRYO_CHAMBER_REGISTRY` overrides). `cryo start` records chambers, and Cryohub merges/prunes this registry unless started with `--local-only`. |
