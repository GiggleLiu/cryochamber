...
1. **Every wake produces at least one visible message to the user.** If
   ...
   *Scope:* a "wake" here is a wake that starts a session. A scheduled
   ...
   Bootstrap, inbox, and forced wakes always start a session.
...
- **Interactive mode via `cryo-agent receive --wait`**: the daemon parks the request inside the active session and delivers the next inbox batch into the same agent process (or a timeout notice telling it to hibernate). Activation is the agent's per-wake choice; the conversation ends in natural language (the agent calls `hibernate`). The session deadline is suspended while parked and reset per round. Default wait 14400 s (`wait_timeout` in cryo.toml), daemon cap 86400 s. Parked state is in-memory only, like `SessionInboxState`. If the parked client dies (e.g. the agent runner's shell timeout kills `cryo-agent`), the daemon detects the socket EOF, frees the parked slot without claiming any inbox batch, and allows a fresh `receive --wait` in the same session — a batch claimed for a dead client would be swallowed and answered by the daemon fallback instead of the agent. The liveness probe runs once per poll tick, so a client that dies in the same tick a batch is being claimed still resolves via the daemon fallback (invariant 2 holds; the no-claim guarantee is best-effort within one tick). A disconnect-reclaim *pauses* the session deadline (shifts it by the parked duration) rather than granting the fresh per-round budget that a real delivery does, so a repeatedly-killed wait client cannot extend a session forever.
- **Scheduled wakes are demand-driven**: the daemon's cached `next_wake` only decides *when to re-check* `todo.json`; whether a session runs is decided by what the wake actually claims. A scheduled wake that claims no due TODO and finds an empty inbox runs nothing — the daemon resyncs `next_wake` from disk and sleeps (pacing retries via `TODO_CLAIM_RETRY_DELAY` if disk still shows past-due work that `claim_due` failed to persist). Bootstrap and inbox/forced wakes are explicit demand and always run. This makes a stale wake-time cache cost a file read instead of an agent session; the 2026-04-21 incident (a pre-fix binary spun ~678 spurious sessions in 9 hours) is the motivating failure shape, reproduced in `test_stale_scheduled_wake_with_no_due_work_skips_session_and_resyncs`.
